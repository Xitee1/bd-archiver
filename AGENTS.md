...
│   ├── xorriso.py      # burn via `xorriso -as cdrecord` (+ DeviceBusyError on drive-acquire failure, SIGINT double-press abort with BURN_ABORT_GRACE_S=5s)
...
│   ├── mediainfo.py    # detect_disc_capacity (Free Blocks for write-once, Track Size for rewritables; assumes xorriso never formats BD-R — no format → no OSA)
...
2. **`burn`** (`commands/burn.py`) iterates `<input>/images/disc_*.iso` lexically and burns each via `xorriso -as cdrecord -v dev=<device> -multi -eject image.iso` — a byte-for-byte ISO write (cdrecord-emulation track source). This replaced the abandoned `growisofs` (dvd+rw-tools, last release 2008), which auto-formats blank BD-R before writing and on some drive+media combos fails to even *start* the burn; `xorriso` does **not** format BD-R, so it sidesteps that failure and gives the old `spare=none` behaviour (no implicit format, no Outer Spare Area, full nominal capacity, full write speed) by default — par2 + sha512 + the post-burn verify pass cover the integrity story at the application layer. The hard rule is to never pass a `blank=`/format command; `-multi` keeps the BD-R **appendable** (growisofs's `-dvd-compat` only closed write-once DVD, never BD-R, so these discs were always appendable — without `-multi` xorriso would silently close them); `-dao` is not passed (no purpose on BD-R, can abort the run). No on-the-fly mkisofs, so what's in the ISO file is exactly what ends up on disc. Volume label, publisher, file layout are all already in the file. Pre-burn fit check is **two-sided**: rejects too-small discs AND discs more than `DISC_OVERSIZE_TOLERANCE` (= 5%) larger than the ISO, guarding against wasting a 50 GB BD-DL on a 25 GB-sized archive. `--skip-fit-check` disables both directions. SIGINT is trapped during the burn itself: a first `Ctrl+C` warns and is ignored (cancelling mid-burn coasters the disc), a second within `BURN_ABORT_GRACE_S` (= 5 s) terminates xorriso and bubbles up as `KeyboardInterrupt`. xorriso runs in its own session (`start_new_session=True`) so the tty's SIGINT does not reach it directly. xorriso's post-burn tray eject is requested explicitly via `-eject` (xorriso does not eject by default) — it's the only reliable way on Linux to invalidate the kernel's cached "Blank BD-R" view of the medium, without which `mount` keeps seeing the pre-burn blank state and udisks2 reports the disc as not-mountable. `DiscIO.wait_for_disc_ready` then waits for the disc to be re-loaded: tray-load drives get `eject -t` (close-tray) retried on the `_CLOSE_TRAY_SCHEDULE_S` schedule (`0, 5, 15, 30, 50` s — five attempts with 5/10/15/20s spacing); slim drives that can't motor the tray fall through to passive polling on `drive_status` (no prompt, no hard timeout — user pushes the disc in, Ctrl+C aborts). The post-burn verify then runs `verify_disc` and loops on any mount/verify failure with a `Re-insert the disc … press Enter to retry` prompt as a last-resort safety net for actual disc corruption. Resumable via `--start N`; per-disc resume hints are logged on every cancel/error path. Catches `DeviceBusyError` from `tools/xorriso.py` (libburn could not acquire the drive) and offers an interactive retry — `tools.lsof.find_device_holders` is consulted to name the holding processes when available.
...
2. **`burn`** (`commands/burn.py`) iterates `<input>/images/disc_*.iso` lexically and burns each via `xorriso -as cdrecord -v dev=<device> -multi -eject image.iso` — a byte-for-byte ISO write (cdrecord-emulation track source), no on-the-fly mkisofs, so what's in the ISO file is exactly what ends up on disc. Volume label, publisher, file layout are all already in the file. This replaced the abandoned `growisofs` (dvd+rw-tools, last release 2008), which auto-formats blank BD-R before writing and on some drive+media combos fails to even *start* the burn; `xorriso` does **not** format BD-R, so it both sidesteps that failure and gives the old `spare=none` behaviour (no Outer Spare Area, full nominal capacity, full speed) by default — the hard rule is to never pass a `blank=`/format command. `-multi` keeps the BD-R **appendable** (growisofs's `-dvd-compat` only closed write-once DVD, never BD-R, so these discs were always appendable — without `-multi` xorriso would silently close them); `-dao` is not passed (no purpose on BD-R, can abort the run). Pre-burn fit check is **two-sided**: rejects too-small discs AND discs more than `DISC_OVERSIZE_TOLERANCE` (= 5%) larger than the ISO, guarding against wasting a 50 GB BD-DL on a 25 GB-sized archive. `--skip-fit-check` disables both directions. SIGINT is trapped during the burn itself: a first `Ctrl+C` warns and is ignored (cancelling mid-burn coasters the disc), a second within `BURN_ABORT_GRACE_S` (= 5 s) terminates xorriso and bubbles up as `KeyboardInterrupt`. xorriso runs in its own session (`start_new_session=True`) so the tty's SIGINT does not reach it directly. xorriso's post-burn tray eject is requested explicitly via `-eject` (xorriso does not eject by default) — it's the only reliable way on Linux to invalidate the kernel's cached "Blank BD-R" view of the medium, without which `mount` keeps seeing the pre-burn blank state and udisks2 reports the disc as not-mountable. `DiscIO.wait_for_disc_ready` then waits for the disc to be re-loaded: tray-load drives get `eject -t` (close-tray) retried on the `_CLOSE_TRAY_SCHEDULE_S` schedule (`0, 5, 15, 30, 50` s — five attempts with 5/10/15/20s spacing); slim drives that can't motor the tray fall through to passive polling on `drive_status` (no prompt, no hard timeout — user pushes the disc in, Ctrl+C aborts). The post-burn verify then runs `verify_disc` and loops on any mount/verify failure with a `Re-insert the disc … press Enter to retry` prompt as a last-resort safety net for actual disc corruption. Resumable via `--start N`; per-disc resume hints are logged on every cancel/error path. Catches `DeviceBusyError` from `tools/xorriso.py` (libburn could not acquire the drive) and offers an interactive retry — `tools.lsof.find_device_holders` is consulted to name the holding processes when available.
...
Raw capacity from `tools.mediainfo.detect_disc_capacity` is `Free Blocks` for write-once media (BD-R, DVD-R — the per-burn writable remainder) and falls back to `Track Size` when `Free Blocks` is 0 (rewritable formatted media — DVD+RW, BD-RE, where `Free Blocks` doesn't apply). This assumes the burn step keeps BD-R unformatted: `xorriso` does not format a blank BD-R by default (see `tools/xorriso.py`), so without a format step no Outer Spare Area is reserved and the nominal `Free Blocks` figure (~25.03 GB on a 25 GB SL BD-R, 12219392 blocks × 2048) is fully writable. If a `blank=`/format command is ever passed to xorriso, the drive will reserve a default OSA (~256 MiB on SL BD-R) and `Free Blocks` will over-report; in that case capacity must instead come from the MMC-6 32h format-type descriptor in `READ FORMAT CAPACITIES`. The two changes are coupled — see commit 43fce62 for the descriptor-walk variant.
