...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ── estimate ────────────────────────────────────────────────────────
    es = sub.add_parser("estimate", help="Estimate disc count + last-disc headroom")
    es.add_argument("-s", "--source", required=True, help="Source directory")
    es.add_argument(
        "-r", "--redundancy", type=int, default=5, help="PAR2 redundancy in %% (default: 5)"
    )
    es.add_argument(
    ...
    ratio_group.add_argument(
        "--sample",
        default=None,
        help="Run dar on this directory with -c/-l and use the measured output/input ratio",
    )
    es.add_argument(
        "-c",
        "--compression",
    ...
    es.add_argument("-l", "--level", help="Compression level for --sample")
    es.add_argument(
        "-w",
        "--workdir",
        default=None,
        help="Directory for sample tempdir (default: $TMPDIR or /tmp — may be tmpfs/RAM)",
    )
