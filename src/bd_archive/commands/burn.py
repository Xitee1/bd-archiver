...
def cmd_burn(args):
    ...
    for i in range(start, disc_count + 1):
        ...
        prompt_disc(f"Insert blank disc {i}/{disc_count}", device)
        ...
        if not args.skip_fit_check:
            actual = detect_disc_capacity(device)
            if actual is None:
        ...
        while True:
            try:
                ...
                log.error(
                    f"Optical device {device} is locked by "
                    f"another process (growisofs couldn't grab "
                    f"the associated sg device)."
                )
                sg = find_sg_device(device)
                holders = find_device_holders(device, sg)
                if holders:
